<template>
  <div class="p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <div class="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between mb-4">
          <div class="w-12 h-12 rounded-full bg-gold-400/20 flex items-center justify-center">
            <GiftIcon class="w-6 h-6 text-gold-500" />
          </div>
          <span class="text-xs font-medium text-gold-600 bg-gold-50 px-2 py-1 rounded-full">Année {{ currentYear }}</span>
        </div>
      ...
      <div class="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow" :class="stats.solde >= 0 ? 'ring-2 ring-green-400' : 'ring-2 ring-red-400'">
        <div class="flex items-center justify-between mb-4">
          <div class="w-12 h-12 rounded-full bg-navy-500/10 flex items-center justify-center">
            <ScaleIcon class="w-6 h-6 text-navy-500" />
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div class="bg-white rounded-xl p-6 shadow-sm">
        <div class="flex justify-between items-center mb-6">
          ...
          <select v-model="selectedYear" @change="loadEvolution" class="text-sm border rounded-lg px-3 py-2 focus:ring-2 focus:ring-gold-400 focus:border-gold-400">
...
const chartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        label: 'Cotisations',
        data: evolutionData.value.map(d => d.cotisations),
        borderColor: '#27AE60',
      ...
      {
        label: 'Dons',
        data: evolutionData.value.map(d => d.dons),
        borderColor: '#D4AF37',
      ...
      {
        label: 'Dépenses',
        data: evolutionData.value.map(d => d.depenses),
        borderColor: '#E74C3C',
...
const doughnutData = computed(() => {
  ...
  return {
    labels: ['Transport', 'Matériel', 'Événement', 'Autre'],
    datasets: [{
      data: [categories.transport, categories.materiel, categories.evenement, categories.autre],
...
const loadEvolution = async () => {
  try {
    const { data } = await dashboardService.getEvolution(selectedYear.value)
